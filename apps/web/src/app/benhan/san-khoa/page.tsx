...
export default function SanKhoaPage() {
  const {
    formData,
    calculated,
    updateField,
    ...
    tomtatCombined,
    setTomtatUser,
  } = useMedicalForm({ persistenceKey: "san-khoa" });
  const [showPreview, setShowPreview] = useState(false);

  const handleExport = async () => {
    const data = getExportData();
    clearSaved();
    await generateNoiKhoaDocx(data, `${data.hoten || "benhan-sankhoa"}.docx`);
  };
  ...
  const previewHtml = buildHTMLPreview(getExportData(), "BỆNH ÁN SẢN KHOA");
