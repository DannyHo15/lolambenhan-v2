...
export default function NhiKhoaPage() {
  const { formData, calculated, updateField, resetForm, getExportData, lastSaved, clearSaved, tomtatCombined, setTomtatUser } = useMedicalForm({ persistenceKey: "nhi-khoa" });
  const [showPreview, setShowPreview] = useState(false);

  const handleExport = async () => {
    const data = getExportData();
    clearSaved();
    await generateNoiKhoaDocx(data, `${data.hoten || "benhan-nhikhoa"}.docx`);
  };
  ...
  const previewHtml = buildHTMLPreview(getExportData(), "BỆNH ÁN NHI KHOA");
        ...
        <form onSubmit={(e) => e.preventDefault()} className="form-container-glass">
          <FormSection title="A. PHẦN HÀNH CHÁNH">
            <FormRow>
              ...
              <div>
                <FormInput id="namsinh" label="3. Năm sinh" type="number" value={formData.namsinh || ""} onChange={(v) => updateField("namsinh", v)} placeholder="2010" className="mb-1" />
