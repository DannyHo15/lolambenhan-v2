...
export default function TienPhauPage() {
  ...
  const handleExport = async () => {
    const data = { ...getExportData(), benhngoai: benhNgoaiKhoa } as any;
    clearSaved();
    await generateNoiKhoaDocx(data, `${data.hoten || "benhan-tienphau"}.docx`);
  };
  ...
  const handleTomtatChange = (value: string) => {
    // Remove the auto-generated part to get user-added content only
    ...
    if (value.startsWith(autoPart)) {
      // User hasn't deleted the auto part, save only the user-added portion
      setTomtatUser(value.slice(autoPart.length));
    } else {
      // User deleted the auto part, save everything as user-added
      setTomtatUser(value);
    }
  };
  ...
  const previewHtml = buildHTMLPreview(getExportData(), "BỆNH ÁN TIỀN PHẪU");
        ...
        <form onSubmit={(e) => e.preventDefault()} className="form-container-glass">
          ...
          <FormSection title="B. PHẦN BỆNH ÁN">
            ...
            <div className="space-y-4 mb-6">
              ...
              <FormTextarea
                id="benhngoai"
                label="2. Bệnh ngoại khoa"
                placeholder="Triệu chứng thực thể ngoại khoa"
                value={benhNgoaiKhoa}
                onChange={(v) => setBenhNgoaiKhoa(v)}
                rows={4}
              />
