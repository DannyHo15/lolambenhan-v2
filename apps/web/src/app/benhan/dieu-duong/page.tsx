...
export default function DieuDuongPage() {
  const {
    formData,
    calculated,
    updateField,
    ...
    tomtatCombined,
    setTomtatUser,
  } = useMedicalForm({ persistenceKey: "dieu-duong" });
  const [showPreview, setShowPreview] = useState(false);

  const handleExport = async () => {
    const data = getExportData();
    clearSaved();
    await generateNoiKhoaDocx(data, `${data.hoten || "benhan-dieuduong"}.docx`);
  };
  ...
  const previewHtml = buildHTMLPreview(getExportData(), "BỆNH ÁN ĐIỀU DƯỠNG");
