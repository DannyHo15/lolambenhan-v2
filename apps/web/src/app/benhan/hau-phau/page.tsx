...
const TIM_MACH_OPTIONS = [
  { value: "", label: "-- Chọn --" },
  { value: "T1 T2 đều rõ, không âm thổi", label: "T1 T2 đều rõ, không âm thổi" },
  {
    value:
...
const HO_HAP_OPTIONS = [
  { value: "", label: "-- Chọn --" },
  { value: "Rì rào phế nang êm dịu 2 phế trường, không ran", label: "Rì rào phế nang êm dịu 2 phế trường, không ran" },
  {
    value:
...
const TIEU_HOA_OPTIONS = [
  { value: "", label: "-- Chọn --" },
  { value: "Bụng mềm, gan lách không sờ chạm, không diểm đau khu trú", label: "Bụng mềm, gan lách không sờ chạm, không diểm đau khu trú" },
  {
    value:
...
const THAN_KINH_OPTIONS = [
  { value: "", label: "-- Chọn --" },
  { value: "Cổ mềm, không dấu thần kinh khu trú", label: "Cổ mềm, không dấu thần kinh khu trú" },
  {
    value:
...
export default function HauPhauPage() {
  const { formData, calculated, updateField, resetForm, getExportData, lastSaved, clearSaved, tomtatCombined, setTomtatUser } = useMedicalForm({ persistenceKey: "hau-phau" });
  const [showPreview, setShowPreview] = useState(false);

  const handleExport = async () => {
    const data = getExportData();
    clearSaved();
    await generateNoiKhoaDocx(data, `${data.hoten || "benhan-hauphau"}.docx`);
  };
  ...
  const previewHtml = buildHTMLPreview(getExportData(), "BỆNH ÁN HẬU PHẪU");
      ...
      <main className="max-w-5xl mx-auto">
        ...
        <form onSubmit={(e) => e.preventDefault()} className="form-container-glass">
                ...
                <FormRow>
                  {["timmach", "hopho", "tieuhoa"].map((field, i) => (
                    <div key={field} className="space-y-2">
                      ...
                      <FormSelect id={`${field}Select`} label="" value="" onChange={(v) => handleSelectToTextarea(v, field as any)} options={[{ value: "", label: "-- Chọn --" }, ...(i === 0 ? TIM_MACH_OPTIONS : i === 1 ? HO_HAP_OPTIONS : TIEU_HOA_OPTIONS).slice(1)]} />
                ...
                <FormRow className="mt-4">
                  {["than", "thankinh", "cokhop"].map((field, i) => (
                    <div key={field} className="space-y-2">
                      ...
                      <FormSelect id={`${field}Select`} label="" value="" onChange={(v) => handleSelectToTextarea(v, field as any)} options={[{ value: "", label: "-- Chọn --" }, ...(i === 0 ? THAN_OPTIONS : i === 1 ? THAN_KINH_OPTIONS : CO_KHOP_OPTIONS).slice(1)]} />
                      <FormTextarea id={field} label="" value={formData[field as keyof typeof formData] || ""} onChange={(v) => updateField(field as keyof typeof formData, v)} rows={3} />
                    </div>
                  ))}
                </FormRow>
          ...
          <FormSection title="C. PHẦN BIỆN LUẬN">
            <FormTextarea id="bienluan" label="" value={formData.bienluan || ""} onChange={(v) => updateField("bienluan", v)} rows={6} />
          </FormSection>
        </form>
      </main>
