...
export default function YHCTPage() {
  const {
    formData,
    calculated,
    updateField,
    ...
    tomtatCombined,
    setTomtatUser,
  } = useMedicalForm({ persistenceKey: "yhct" });
  const [showPreview, setShowPreview] = useState(false);

  const handleExport = async () => {
    const data = getExportData();
    clearSaved();
    await generateNoiKhoaDocx(data, `${data.hoten || "benhan-yhct"}.docx`);
  };
