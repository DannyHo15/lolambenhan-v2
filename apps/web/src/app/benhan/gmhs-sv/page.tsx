...
export default function GMHSSVPage() {
  const {
    formData,
    calculated,
    updateField,
    ...
    tomtatCombined,
    setTomtatUser,
  } = useMedicalForm({ persistenceKey: "gmhs-sv" });
  const [showPreview, setShowPreview] = useState(false);

  const handleExport = async () => {
    const data = getExportData();
    clearSaved();
    await generateNoiKhoaDocx(data, `${data.hoten || "benhan-gmhs-sv"}.docx`);
  };
  const handlePreview = () => setShowPreview(true);
  const handleReset = () => {
    if (confirm("Xoá hết dữ liệu trong form?")) resetForm();
  };
  ...
  const previewHtml = buildHTMLPreview(
    getExportData(),
    "BỆNH ÁN GÂY MÊ HỒI SỨC (SINH VIÊN)",
  );
    ...
    <div className="min-h-screen bg-transparent pt-0 pb-8 px-4">
      ...
      <main className="max-w-5xl mx-auto">
        ...
        <div className="text-center mb-8">
          ...
          >
            Form BỆNH ÁN GÂY MÊ HỒI SỨC (SINH VIÊN)
          </h1>
                  ...
                  {["timmach", "hopho", "tieuhoa"].map((field, i) => (
                    <div key={field} className="space-y-2">
                      <label className="text-xs text-muted-foreground font-medium">
                        {["a) Tuần hoàn:", "b) Hô hấp", "c) Tiêu hoá:"][i]}
                      </label>
                      <FormSelect
                        id={`${field}Select`}
                        label=""
                        value=""
                        onChange={(v) =>
                          handleSelectToTextarea(v, field as any)
                        }
                        options={
                          i === 0
                            ? TIM_MACH_OPTIONS
                      ...
                      <FormTextarea
                        ...
                        onChange={(v) =>
                          updateField(field as keyof typeof formData, v)
                        }
                        rows={3}
                      />
                    </div>
